    ...
    return {
        ...
        playbooks: {
            ...
            organicMarketing: [
                ...
                "Priorizar Canva Pro para arte e Buffer/Meta Business Suite para agendamento sem custo extra.",
        ...
        freeGrowthStack: [
            ...
            },
            {
                name: "Canva Pro",
                cost: "ja contratado",
                use: "montar artes com IA, templates e recursos Pro a partir dos prompts gerados pela retaguarda"
            }
        ],
...
function polishMarketingPortuguese(value = "") {
    ...
    const phraseReplacements = [
        ["nao e decoreba", "n\u00e3o \u00e9 decoreba"],
        ["Nao e decoreba", "N\u00e3o \u00e9 decoreba"],
        ["n\u00e3o e decoreba", "n\u00e3o \u00e9 decoreba"],
        ["N\u00e3o e decoreba", "N\u00e3o \u00e9 decoreba"],
        ["casos favoraveis", "casos favor\u00e1veis"],
        ["Casos favoraveis", "Casos favor\u00e1veis"],
        ["moral e o conjunto", "moral \u00e9 o conjunto"],
        ["Moral e o conjunto", "Moral \u00e9 o conjunto"],
        ["mais facil", "mais f\u00e1cil"],
        ["Mais facil", "Mais f\u00e1cil"],
        ["esta no texto", "est\u00e1 no texto"],
        ["Esta no texto", "Est\u00e1 no texto"],
        ["nao precisa ser dificil", "n\u00e3o precisa ser dif\u00edcil"],
        ["Nao precisa ser dificil", "N\u00e3o precisa ser dif\u00edcil"],
        ["n\u00e3o precisa ser dificil", "n\u00e3o precisa ser dif\u00edcil"],
        ["N\u00e3o precisa ser dificil", "N\u00e3o precisa ser dif\u00edcil"]
    ];
    ...
    const phrased = phraseReplacements.reduce((text, [plain, accented]) => (
        text.replace(new RegExp(plain, "g"), accented)
    ), value);
...
function getDefaultMarketingContentState() {
    return {
        ...
        strategy: {
            ...
            freeTools: ["Canva Pro", "Buffer Free", "Meta Business Suite", "Instagram scheduler nativo"]
        },
        items: [],
    ...
    return [
        {
            ...
            category: "criacao",
            status: "ready",
            summary: "Pronto para criar artes com IA/templates usando os prompts do painel.",
            checks: [
                "Canva Pro ja contratado",
                "Prompt, legenda e briefing saem do painel"
            ],
            nextAction: "Clicar em Automatizar gratis, abrir Canva IA e revisar a arte."
        },
        {
            key: "buffer",
            name: "Buffer",
            ...
            nextAction: bufferToken && !bufferProfileIds
                ...
                : "Transformar item pronto em post agendado."
        }
    ];
}
...
function buildIntegrationDraft(item, target) {
    ...
    const format = String(item.format || "post").trim();
    const formatLabels = {
        quiz_post: "post quadrado para Instagram, 1080x1080",
        carousel: "carrossel para Instagram, 5 slides, 1080x1080",
        reel_script: "capa de Reels vertical, 1080x1920",
        story_quiz: "story vertical para Instagram, 1080x1920"
    };
    const canvaFormat = formatLabels[format] || "post para Instagram";
    const textForArt = [
        title.replace(/^Post quiz:\s*/i, "").replace(/^Carrossel:\s*/i, "").replace(/^Reel:\s*/i, "").replace(/^Story quiz:\s*/i, ""),
        item.hook || ""
    ].map((line) => polishMarketingPortuguese(line).trim()).filter(Boolean).slice(0, 2).join("\n");
...
async function prepareNoCostMarketingPack(input = {}) {
    const itemId = String(input.itemId || input.id || "").trim();
    if (!itemId) {
        return {
            ok: false,
    ...
    const items = safeArray(current.items).map((item) => {
        ...
        const existingDrafts = item.integrationDrafts && typeof item.integrationDrafts === "object"
            ? item.integrationDrafts
            : {};
        canvaDraft = {
            ...buildIntegrationDraft(item, "canva"),
            preparedAt: new Date().toISOString()
        };
        const baseBufferDraft = existingDrafts.buffer || buildIntegrationDraft(item, "buffer");
        const dueAt = toBufferDueAt(input.dueAt || baseBufferDraft.dueAt);
        const text = polishMarketingPortuguese(input.text || baseBufferDraft.text || item.caption || "");
        const lastResults = channelIds.length
            ? channelIds.map((channelId) => ({
                ok: true,
                channelId,
                status: "dry_run",
                message: "Teste seguro: o Buffer receberia texto e horario. Nada foi enviado."
            }))
            : [{
                ok: false,
                channelId: "",
                status: "buffer_channels_missing",
                message: "Buffer ainda precisa de canal configurado para agendar automaticamente."
            }];

        bufferDraft = {
            ...baseBufferDraft,
            status: channelIds.length ? "test_ready" : "prepared",
            dueAt,
            text,
            scheduledPosts: baseBufferDraft.scheduledPosts || [],
            lastResults,
            testedAt: channelIds.length ? new Date().toISOString() : baseBufferDraft.testedAt || "",
            preparedAt: baseBufferDraft.preparedAt || new Date().toISOString()
        };
    ...
    await setStateValue(MARKETING_CONTENT_STATE_KEY, next);
    await insertAuditLog({
        eventType: "marketing_no_cost_pack_prepared",
        actor: "ops_console",
        targetSystem: "papiro_marketing",
        entityType: "content_item",
        entityId: itemId,
        status: channelIds.length ? "test_ready" : "prepared",
        metadata: {
            canvaPrepared: Boolean(canvaDraft),
            bufferPrepared: Boolean(bufferDraft),
            bufferChannels: channelIds.length
        }
    });

    return {
        ok: true,
        status: channelIds.length ? "no_cost_pack_test_ready" : "no_cost_pack_prepared",
        message: channelIds.length
            ? "Pacote gratis preparado: Canva Pro, legenda, Buffer e teste seguro."
            : "Pacote gratis preparado. Configure canais do Buffer para testar agendamento automatico.",
        draft: {
            canva: canvaDraft,
            buffer: bufferDraft
        },
...
function buildBufferAssetsInput(imageUrl = "") {
    ...
    assets: {
      images: [
        {
          url: ${JSON.stringify(cleanImageUrl)}
        }
      ]
    }`;
}

async function createBufferPost({ channelId, text, dueAt, imageUrl }) {
    const assetsInput = buildBufferAssetsInput(imageUrl);
    const query = `
mutation CreatePost {
  createPost(input: {
...
async function scheduleMarketingContentWithBuffer(input = {}) {
    const itemId = String(input.itemId || input.id || "").trim();
    const dryRun = Boolean(input.dryRun || input.dry_run || input.testOnly);
    const force = Boolean(input.force || input.confirmDuplicate);
    const imageUrl = String(input.imageUrl || input.image_url || "").trim();
    if (!itemId) {
        return {
            ok: false,
    ...
    const alreadyScheduled = draft.status === "scheduled" && Array.isArray(draft.scheduledPosts) && draft.scheduledPosts.length;
    if (!text) {
        return {
            ok: false,
    ...
    if (imageUrl && !/^https:\/\//i.test(imageUrl)) {
        return {
            ok: false,
            status: "buffer_image_url_invalid",
            message: "A arte precisa ser uma URL HTTPS publica para o Buffer conseguir usar."
        };
    }
    if (alreadyScheduled && !force && !dryRun) {
        return {
            ok: false,
            status: "buffer_duplicate_guard",
            message: "Este conteudo ja parece agendado no Buffer. Use forcar apenas se tiver certeza que quer duplicar."
        };
    }

    if (dryRun) {
        const simulatedResults = channelIds.map((channelId) => ({
            ok: true,
            channelId,
            status: "dry_run",
            message: imageUrl
                ? "Teste seguro: o Buffer receberia texto, horario e arte. Nada foi enviado."
                : "Teste seguro: o Buffer receberia texto e horario. Nada foi enviado."
        }));
        const nextItems = safeArray(current.items).map((entry) => {
            if (String(entry.id) !== itemId) {
                return entry;
            ...
            return {
                ...
                integrationDrafts: {
                    ...
                    buffer: {
                        ...
                        text,
                        imageUrl,
                        scheduledPosts: draft.scheduledPosts || [],
                        lastResults: simulatedResults,
        ...
        await setStateValue(MARKETING_CONTENT_STATE_KEY, next);
        await insertAuditLog({
            eventType: "marketing_buffer_dry_run",
            actor: "ops_console",
            targetSystem: "buffer",
            entityType: "content_item",
            entityId: itemId,
        ...
        return {
            ok: true,
            status: "buffer_dry_run_ready",
            message: "Teste seguro concluido. Nada foi enviado ao Buffer.",
            ...next
        };
    }
    ...
    for (const channelId of channelIds) {
        results.push({
            channelId,
            ...(await createBufferPost({ channelId, text, dueAt, imageUrl }))
        });
    }
