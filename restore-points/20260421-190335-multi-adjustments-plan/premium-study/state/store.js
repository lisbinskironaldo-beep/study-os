    ...
    window.PremiumStudyStore = {
        ...
        applyGeneratedBundle(payload = {}) {
            ...
            const blocks = sourceBlocks.map((block, index) => {
                const practice = block.practice || {};
                const quizSeries = Array.isArray(practice.quizSeries) && practice.quizSeries.length
                    ? practice.quizSeries
                    : [practice.quiz || []];
                const trueFalseSeries = Array.isArray(practice.trueFalseSeries) && practice.trueFalseSeries.length
                    ? practice.trueFalseSeries
                    : [practice.trueFalse || []];
                const flashcardSeries = Array.isArray(practice.flashcardSeries) && practice.flashcardSeries.length
                    ? practice.flashcardSeries
                    : [practice.flashcards || []];
                ...
                return {
                    ...
                    practice: {
                        ...
                        ...practice,
                        quiz: Array.isArray(quizSeries[0]) ? quizSeries[0] : [],
                        quizSeries,
                        trueFalse: Array.isArray(trueFalseSeries[0]) ? trueFalseSeries[0] : [],
                        trueFalseSeries,
                        flashcards: Array.isArray(flashcardSeries[0]) ? flashcardSeries[0] : [],
                        flashcardSeries
                    },
                    exam: {
                        ...(block.exam || {}),
