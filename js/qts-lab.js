const QTS = {
    ...
    data: JSON.parse(localStorage.getItem("qts_lab_v1")) || {
        studyDuration: 25,
    ...
    getSubjectDisplayMap() {
        return {
            ...
            ciencias: "CiÍncias",
            educacaofisica: "EducaÁ„o FÌsica"
        };
    },
            ...
            container.innerHTML = `
                <div class="qts-summary-card is-empty">
                    <div class="qts-summary-head">
                        <div>
                            <span class="qts-summary-kicker">Planejado por matÈria</span>
                            <strong>Sem matÈrias distribuÌdas ainda.</strong>
                        </div>
                    </div>
            ...
            <div class="qts-summary-card${expanded ? " is-expanded" : ""}">
                <button type="button" class="qts-summary-toggle" aria-expanded="${expanded ? "true" : "false"}">
                    <span class="qts-summary-toggle-copy">
                        <span class="qts-summary-kicker">Planejado por matÈria</span>
                        <strong>${summary.length} matÈrias em ${scopeMeta}</strong>
                    </span>
                ...
                <div class="qts-summary-body"${expanded ? "" : " hidden"}>
                    <div class="qts-summary-head">
                    <div>
                        <span class="qts-summary-kicker">DistribuiÁ„o atual</span>
                        <strong>${summary.length} matÈrias em ${scopeMeta}</strong>
                    </div>
            ...
            <div class="qts-stage-shell">
                <div class="qts-stage-main">
                    <div class="qts-template-strip">
                        <div class="qts-template-label" aria-hidden="true">MÈtodo pomodoro</div>
        ...
        rail.innerHTML = `
            <section class="qts-detached-block is-week" aria-label="Semanas">
                ...
                <div class="qts-detached-week-nav" role="group" aria-label="Alternar semana">
                    ...
                    <button type="button" class="qts-detached-week-button${weekOffset === 0 ? " is-active" : ""}" data-week-nav="0" data-compact-icon="ï" aria-label="Semana atual" title="Semana atual" aria-pressed="${weekOffset === 0}">
                        <span class="qts-detached-icon" aria-hidden="true">ï</span>
                        <span class="qts-detached-text">Atual</span>
                    </button>
                    <button type="button" class="qts-detached-week-button${weekOffset === 1 ? " is-active" : ""}" data-week-nav="1" data-compact-icon="?" aria-label="PrÛxima semana" title="PrÛxima semana" aria-pressed="${weekOffset === 1}">
                        <span class="qts-detached-icon" aria-hidden="true">?</span>
                        <span class="qts-detached-text">PrÛxima</span>
                    </button>
                </div>
            </section>
            <div class="qts-detached-index-divider" aria-hidden="true"></div>
            <section class="qts-detached-block is-toggle" aria-label="Dias e hor·rio">
                <div class="qts-detached-block-label">Dias e hor·rio</div>
                <div class="qts-detached-toggle-group" role="group" aria-label="Opcoes da tabela">
                    <label class="qts-detached-toggle" data-compact-icon="?" aria-label="Mostrar hor·rio" title="Mostrar hor·rio">
                    ...
                    <label class="qts-detached-toggle" data-compact-icon="?" aria-label="Mostrar s·bado" title="Mostrar s·bado">
            ...
            <section class="qts-detached-block is-summary" aria-label="Planejamento">
                ...
                <button type="button" class="qts-detached-summary-button${this.data.summaryVisible ? " is-active" : ""}" data-summary-toggle data-compact-icon="?" aria-label="Mostrar planejamento por matÈria" title="Mostrar planejamento por matÈria" aria-pressed="${this.data.summaryVisible}">
                    <span class="qts-detached-icon" aria-hidden="true">?</span>
                    <span class="qts-detached-text">Por matÈria</span>
                </button>
            </section>
        `;
    },
...
// BAIXO ‚Üí vai pra pr√≥xima linha v√°lida
if(e.key==="ArrowDown"){
e.preventDefault()
...
// DIREITA (vai para primeira c√©lula da linha)
if(e.key==="ArrowRight"){
e.preventDefault()
...
// se N√ÉO est√° no come√ßo ‚Üí n√£o sai
...
// pega a √∫ltima c√©lula v√°lida da linha
...
// se n√£o achou hor√°rio, tenta c√©lula normal
if(!next){
next = document.querySelector(
...
// for√ßa leitura correta independente de nodes
...
return {
start: endStudy,   // in√≠cio da pausa
...
const title = rawTitle || "Bloco sem tÌtulo"
const studyMinutes =
