...
async function validateGoogleAuth(env) {
    const clientId = env.GOOGLE_CLIENT_ID || env.ROTANOTA_GOOGLE_CLIENT_ID || "";
    const sessionSecretReady = isFilled(env.ROTANOTA_AUTH_SECRET) || isFilled(env.OPS_SESSION_SECRET);
...
async function main() {
    ...
    if (!isFilled(env.GOOGLE_CLIENT_ID) && !isFilled(env.ROTANOTA_GOOGLE_CLIENT_ID)) {
    ...
    if (!isFilled(env.ROTANOTA_AUTH_SECRET)) {
        warnings.push("ROTANOTA_AUTH_SECRET ainda nao foi preenchido. O login usa fallback do painel, mas o ideal e usar um segredo proprio para a conta.");
    }
    ...
    printSection("Passos locais", [
        ...
        "- configurar `GOOGLE_CLIENT_ID` e `ROTANOTA_AUTH_SECRET` antes de testar login real",
